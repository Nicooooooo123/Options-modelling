...
# ui_components.py
def create_sidebar_inputs(risk_free_rate: float, dividend_yield: float):
    """Sidebar controls (rates are read‑only metrics)."""
    st.sidebar.header("Model Parameters")

    # Display live rates
    st.sidebar.metric("Risk‑Free Rate (2‑Y UST)", f"{risk_free_rate*100:.2f}%")
    ...
    option_data_type = st.sidebar.radio(
        ...
        index=0  # default to Calls Only on page load
    )

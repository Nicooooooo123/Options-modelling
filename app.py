...
with col1:
    st.image("vegadelta_logo.png", width=200)
with col2:
...
with st.container():
    ...
    try:
        ...
        # --- Options-data quality flag based on number of expirations ---
        ...
        # Sometimes Yahoo already gives the value in % (e.g. 0.51 = 51%).
        # If the number looks implausibly high, scale it down.
        if dividend_yield > 0.20:
            dividend_yield /= 100.0

        # 2‑year Treasury yield as risk‑free rate ---------------------------
        try:
            two_year = yf.Ticker('2YY=F')
            rf_hist = two_year.history(period='1d')['Close']
            if rf_hist.empty:
                rf_hist = two_year.history(period='5d')['Close']
            risk_free_rate = rf_hist.iloc[-1] / 100.0  # 3.75 ➜ 0.0375
        except Exception:
            risk_free_rate = 0.03  # sensible default
...
# -----------------------------------------------------------------------------
# Sidebar controls (need the live rates first)
# -----------------------------------------------------------------------------
option_type, option_data_type = ui.create_sidebar_inputs(risk_free_rate, dividend_yield)
...
with st.spinner('Working hard or hardly working...'):
    svi_df = svi.generate_svi_calibrated_data(unique_maturities, imp_vol_data,
                                              spot_price, reg_weight, multi_start_count,
                                              option_data_type)
